...
arduino = serial.Serial(port="COM3", baudrate=9600, timeout=1)  # Adjust COM port if needed
...
def detect_biodegradable(frame):
    ...
    # If exactly three objects are detected, move the plate to the next position
    # if region_count == 3:
...
def process_second_third_layer(cam_2):
    ...
    for _ in range(4):
        ...
        if region_count == 0:
            ...
            print(f"Biodegradable: {bio_count}, Non-Biodegradable: {non_bio_count}")
            ...
            if bio_count == region_count:
                ...
                send_command("MOVE_MULTI")
                time.sleep(2)
            elif bio_count == region_count & non_bio_count == region_count:
